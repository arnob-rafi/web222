...
function extractTimeZones(data) {
  ...
  observationsData.forEach((observation) => {
    ...
    if (createdTimeZone && !uniqueTimeZones.includes(createdTimeZone)) {
      uniqueTimeZones.push(createdTimeZone);
    }

    if (observedTimeZone && !uniqueTimeZones.includes(observedTimeZone)) {
      uniqueTimeZones.push(observedTimeZone);
    }
  });
